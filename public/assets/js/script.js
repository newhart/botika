...
function readURL(uploader) {
    ...
    $.ajax({
        url: `/client/add-profile/${user_id}`, // Assurez-vous d'avoir une route correspondante dans votre fichier routes/web.php
        type: "POST",
        data: formData,
        headers: {
...
$(".close_button").click(function (e) {
    e.preventDefault;
    const user_id = document.getElementById("auth_id").value;
    const csrfToken = $('meta[name="csrf-token"]').attr("content");
    const formData = new FormData();
    formData.append("id", user_id);
    $.ajax({
        url: `/client/${user_id}/delete`, // Assurez-vous d'avoir une route correspondante dans votre fichier routes/web.php
        type: "DELETE",
        data: formData,
        headers: {
            "X-CSRF-TOKEN": csrfToken,
        ...
        contentType: false,
        success: function (response) {
            console.log("remove with success", response);
        },
        error: function (error) {
            console.log("an error exist", error);
        },
    });
    $(this)
...
// form validation
$("#update-address").submit(function (event) {
    ...
    const pin_code = $("#pin_code").val().trim();
    const csrfToken = $('meta[name="csrf-token"]').attr("content");

    // Réinitialisez les classes invalides
    ...
    // Si la validation est réussie, envoyez une requête AJAX
    $.ajax({
        url: `client/address/${address_id}/edit`,
        type: "POST",
        data: {
            author,
            first_name,
